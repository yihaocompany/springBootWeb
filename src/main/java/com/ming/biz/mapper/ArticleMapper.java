...
public interface ArticleMapper {
    ...
    @Insert("insert into article(articleId,author,originalAuthor," +
            "articleTitle,publishDate,updateDate,articleContent," +
            "articleTags,articleType,articleCategories,articleUrl," +
            "articleTabloid,likes,lastArticleId,nextArticleId) " +
            "values(#{articleId},#{author},#{originalAuthor}," +
            "#{articleTitle},#{publishDate},#{updateDate}," +
            "#{articleContent},#{articleTags},#{articleType}," +
            "#{articleCategories},#{articleUrl},#{articleTabloid}," +
            "#{likes},#{lastArticleId},#{nextArticleId})")
    void insertArticle(Article article);

    @Update("update article set originalAuthor=#{originalAuthor}," +
            "articleTitle=#{articleTitle},updateDate=#{updateDate}," +
            "articleContent=#{articleContent},articleTags=#{articleTags}," +
            "articleType=#{articleType},articleCategories=#{articleCategories}," +
            "articleUrl=#{articleUrl},articleTabloid=#{articleTabloid} where id=#{id}")
    ...
    @Select("select articleId,originalAuthor,articleTags," +
            "articleTitle,articleType,publishDate,originalAuthor," +
            "articleCategories,articleTabloid,likes from article " +
            "order by id desc")
    ...
    @Select("select articleId,originalAuthor,articleTitle," +
            "articleTags,articleType,articleCategories,publishDate " +
            "from article where articleTags like '%${tag}%' order by id desc")
    List<Article> findArticleByTag(@Param("tag") String tag);

    @Select("select articleId,originalAuthor,articleTitle,articleTags," +
            "articleType,articleCategories,publishDate from article " +
            "where articleCategories=#{category} order by id desc")
    List<Article> findArticleByCategory(@Param("category") String category);

    @Select("select articleId,originalAuthor,articleTitle,articleTags," +
            "articleType,articleCategories,publishDate from article " +
            "where publishDate like '%${archive}%' order by id desc")
    List<Article> findArticleByArchive(@Param("archive") String archive);

    @Select("select articleId,originalAuthor,articleTitle,articleTags," +
            "articleType,articleCategories,publishDate from article order by id desc")
    List<Article> findAllArticlesPartInfo();

    @Select("select id,articleId,originalAuthor,articleTitle," +
            "articleCategories,publishDate from article order by id desc")
    List<Article> getArticleManagement();

    @Select("select id,articleId,originalAuthor,articleTitle," +
            "articleContent,articleCategories,articleTags,articleType,articleUrl from article where id=#{id}")
